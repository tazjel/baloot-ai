...
class BiddingEngine:
    ...
    def _handle_contract_bid(self, player_idx, action, suit):
        ...
        if self.contract.type == BidType.SUN:
             ...
             if action in ["SUN", "ASHKAL"]:
                  # Trying to bid Sun over Sun? 
                  # Only allowed if higher priority (Gablak/Hijack).
                  # Handled below by Priority Check.
                  pass

         # Ensure Ashkal logic doesn't crash in R2 checking floor card suit match (R2 has no floor match constraint for Sun)
        ...
        if self.phase == BiddingPhase.GABLAK_WINDOW:
             ...
             # Only HIGHER priority than tentative bidder allowed to act here
             ...
             if action == "PASS":
                  # If I am the highest priority remaining, and I pass -> Close window?
                  # Simpler: Just ignore Pass in window? Or treat as "No Steal".
                  # Let's verify priority and just return success (no change).
    ...
    def _finalize_tentative_bid(self):
        ...
        if is_sun:
             ...
             self.phase = BiddingPhase.ROUND_1 # Restore (Logic needs to know if R1 or R2)
             # How to know? State machine needs to remember prev state?
             # Simplified: If floor card suit match -> R1. Else R2.
             pass
             
        self.tentative_bid = None
        # Advance turn from the bidder?
        self.current_turn = (tb['bidder'] + 1) % 4
    ...
    def _set_contract(self, player_idx, action, suit):
        ...
        # Track Round
        self.contract.round = 1 if self.phase == BiddingPhase.ROUND_1 else 2
